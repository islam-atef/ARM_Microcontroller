 ...
 *  Description : This Function Initializes the Pin and Define The
 * {Mode Type},{OutputType} ,{Output Speed} and {PULL_UP PULL_DOWN state}.
 ******************************************************************************************************************/
void qMGPIO_voidMODEInit(PIN_CONFIG *copy_PinConfiguration) {
  ...
  if (copy_PinConfiguration->pin < 16) {
    if ((copy_PinConfiguration->port == GPIO_A &&
         copy_PinConfiguration->pin == 13) ||
        (copy_PinConfiguration->port == GPIO_A &&
         copy_PinConfiguration->pin == 14) ||
        (copy_PinConfiguration->port == GPIO_B &&
         copy_PinConfiguration->pin == 3) ||
        (copy_PinConfiguration->port == GPIO_B &&
         copy_PinConfiguration->pin == 4)) { /* Error */
    } else { /* 			Configure The (MODER) register to the
              * Needed Mode
              */
      (PORT)->MODER &= ~(0b11 << (copy_PinConfiguration->pin * 2));
      (PORT)->MODER |=
          (copy_PinConfiguration->MODEV << (copy_PinConfiguration->pin * 2));

      if (copy_PinConfiguration->MODEV == (OUTPUT || ALTERNATE)) {
        /* 				Configure The (OTYPER) register to the
         * Needed output Type 						  */
        (PORT)->OTYPER &= ~(0b1 << (copy_PinConfiguration->pin));
        (PORT)->OTYPER |=
            (copy_PinConfiguration->OTYPEV << (copy_PinConfiguration->pin));
        /* 				Configure The (OSPEEDR) register to the
         * Needed output speed					  */
        (PORT)->OSPEEDR &= ~(0b11 << (copy_PinConfiguration->pin * 2));
        (PORT)->OSPEEDR |= (copy_PinConfiguration->OSPEEDV
                            << (copy_PinConfiguration->pin * 2));
      }
      /* 			Configure The (PUPDR) register to the Needed
       * Type 								  */
      (PORT)->PUPDR &= ~(0b11 << (copy_PinConfiguration->pin * 2));
      (PORT)->PUPDR |=
          (copy_PinConfiguration->PUPDV << (copy_PinConfiguration->pin * 2));
    }
  }
}
/******************************************************************************************************************/
 ...
 *Output Pins Value.
 *  Returning : Returns Nothing.
 *  Description :  This Function Write The Output Signal Type If The Whole POrt
 *Is used as OUTPUT.
 ******************************************************************************************************************/
 ...
 */
/*  I/P parameters : a pointer To ( PIN_CONFIG ) struct and The Pin Output
 * Signal Type		                      */
/*  Returning	: Returns Nothing
 */
 ...
 */
/*  I/P parameters : 1- a pointer To ( PIN_CONFIG ) struct and The Pin Alternate
 * Function Type.		              */
/*					 2- the type of the Alternate Function
 * that will be operated. */
/*  Returning	   : Returns Nothing.
 */
/*  Description    :       This Function Define The Alternate Function Type For
 * The Pin 						  */
/******************************************************************************************************************/
void MGPIO_voidALTFuncTYPE(PIN_CONFIG *copy_PinConfiguration,
  ...
  if (copy_PinConfiguration->pin < 8) {
    ...
    (PORT)->AFRL |=
        (copy_AlternateFunction << (copy_PinConfiguration->pin * 4));
  } else {
    (PORT)->AFRH &= ~(0b1111 << (((copy_PinConfiguration->pin) - 8) * 4));
    (PORT)->AFRH |=
        (copy_AlternateFunction << (((copy_PinConfiguration->pin) - 8) * 4));
  }
}
/******************************************************************************************************************/
