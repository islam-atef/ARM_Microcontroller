...
/********************************************************************************************/
static void (* USART1_CallBack) (USART_HANDLE *USARTx) = NULL ;
u8  __USART1__INTERRUPT_TYPE__ ;
static void (* USART2_CallBack) (USART_HANDLE *USARTx) = NULL ;
u8  __USART2__INTERRUPT_TYPE__ ;
static void (* USART6_CallBack) (USART_HANDLE *USARTx) = NULL ;
u8  __USART6__INTERRUPT_TYPE__ ;

USART_HANDLE *USART1_Struct;
USART_HANDLE *USART2_Struct;
USART_HANDLE *USART6_Struct;

TIM_HANDLER *TIMX_Handle;
/********************************************************************************************/
...
///@retval Functions Status
Uart_Fun_Status MCAL_UART_Receive( USART_HANDLE *USARTx , u8 *ptData ,u16 Size_Limit , u32 Wait_Time, u8 Last_element)
{
    // Check the Given data and the size values.
    ...
    {
        if(MCAL_TIM_Elapsed_Time(TIMX_Handle) >= Wait_Time)
        {
            // stop the Timer.
            MCAL_TIM_STOP_peri(TIMX_Handle);
    ...
    USARTx -> Time_Limit += MCAL_TIM_Elapsed_Time(TIMX_Handle);

    while (USARTx -> RX_Process_Count <= USARTx -> RX_Buffer_Size)
    {
        ...
        // check the Timer.
        if (MCAL_TIM_Elapsed_Time(TIMX_Handle) == USARTx -> Time_Limit)
        {
            // stop the Timer.
            MCAL_TIM_STOP_peri(TIMX_Handle);
...
/// @param Time_Limit               : the maximum time for this function. 
///@retval Functions Status.
Uart_Fun_Status	    MCAL_UART_Transmit_INT(USART_HANDLE *USARTx , u8 *ptData ,u16 Size ,u8 Last_element)
{
    // Check the Given data and the size values.
    ...
    // Initiate the rest of elements iin the USARTx Struct.
    USARTx -> TX_Buffer_Ptr     = ptData;
    USARTx -> TX_Buffer_Size    = Size;
    USARTx -> TX_Process_Count  = 0;
    USARTx -> Error_Code        = (u8 *)Error_1;
...
/// @brief MCAL_UART_Receive_INT  : this function Receive an amount of data by the Asynchronous mode "Interrupt".
/// @param USARTx                 : the Struct of Peripheral's Registers.
/// @param ptData                 : pointer of data we want to Transmit.
/// @param Size                   : the size of the data that will be Transmitted.
/// @param Time_Limit             : the maximum time for this function.
